...
// Вывод на экран запроса о введении массива, его прием и формат
	...
	chg_var entered_array[100], data; 
	char c;
	int jent = -1;
	do
	{
...
// Вывод массивов Stdout, Stderr, Reduced в потоки и на экран 
	printf("\nStdout: ");
	if (jout == 0) printf("-");
	else { for (int i = 0; i < jout; i++) fprintf(stdout, "%ld ", stdout_array[i]); }

	printf("\nStderr: ");
	if (jerr == 0) printf("-");
	else { for (int i = 0; i < jerr; i++) fprintf(stderr, "%ld ", stderr_array[i]); 	}

	printf("\nReduced: ");
	for (int i = 0; i < jsort; i++) printf("%ld ", reduced_array[i]); 

	bubble_sort(reduced_array, &jsort);  
